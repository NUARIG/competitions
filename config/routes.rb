...
Rails.application.routes.draw do
  ...
  resources :grants do
    ...
    resources :submissions,       except: %i[new], controller: 'grant_submissions/submissions' do
      resources 'applicants',     only: %i[index create destroy],
                                  controller: 'grant_submissions/submissions/submission_applicants'
      ...
      resource 'assign_review', only: %i[new create], controller: 'grant_submissions/submissions/reviews/assign',
                                on: :member
    ...
    resources :reviewers, only: %i[index create destroy], controller: 'grant_reviewers' do
      collection do
        ...
        resources 'invitations',  only: %i[index update destroy], controller: 'grant_reviewers/invitations' do
          collection do
            resources :reminders, only: %i[index show], as: 'invitation_reminders',
                                  controller: 'grant_reviewers/invitations/reminders'
          end
        end
      end

      resources 'reviews', controller: 'grant_reviewers/reviews', on: :member
    end
