...
RSpec.describe 'Profile Grants', type: :system, js: true do
  ...
  let(:grant1)                    do
    create(:open_grant_with_users_and_form_and_submission_and_reviewer,
           name: "First #{Faker::Lorem.sentence(word_count: 3)}")
  end
  ...
  let!(:grant2_viewer_permission) do
    create(:viewer_grant_permission,  grant: grant2,
                                      user: grant1.admins.first)
  end
  let!(:grant3_editor_permission) do
    create(:editor_grant_permission,  grant: grant3,
                                      user: grant1.admins.first)
  end
  let!(:grant4_admin_permission) do
    create(:admin_grant_permission,   grant: grant4,
                                      user: grant1.admins.first)
  end
  ...
  context '#index' do
    context 'grant_admin' do
      before(:each) do
        grant_admin.update(grant_creator: false)
      ...
      context 'manage links' do
        ...
        context 'admin role' do
          scenario 'includes proper links' do
            ...
            expect(page).to have_link 'Panel',        href: edit_grant_panel_path(grant1)
            expect(page).to have_link 'Permissions',  href: grant_grant_permissions_path(grant1)
            expect(page).not_to have_link 'Duplicate', href: new_grant_duplicate_path(grant1)
        ...
        context 'editor role' do
          scenario 'includes proper links' do
            ...
            expect(page).to have_link 'Panel',          href: edit_grant_panel_path(grant3)
            expect(page).to have_link 'Permissions',    href: grant_grant_permissions_path(grant3)
            expect(page).not_to have_link 'Duplicate',  href: new_grant_duplicate_path(grant3)
            expect(page).not_to have_link 'Delete',     href: grant_path(grant3)
          end
        end
        ...
        context 'viewer role' do
          scenario 'includes proper links' do
            ...
            expect(page).to have_link 'Panel',            href: grant_panel_path(grant2)
            expect(page).not_to have_link 'Permissions',  href: grant_grant_permissions_path(grant2)
    ...
    context 'grant_creator and grant_admin' do
      scenario 'includes proper links' do
        login_user grant_admin
        visit profile_grants_path
