  ...
  describe '#index' do
    ...
    context '#index' do
      ...
      scenario 'it includes links to assigned reviews' do
        ...
        expect(page).to have_link(ASSIGNED_TEXT, href: edit_grant_submission_review_path(grant1, grant1.submissions.first, grant1_review))
        expect(page).to have_link(SUBMITTED_TEXT, href: grant_submission_review_path(grant2, grant2.submissions.first, grant2_review))
        expect(page).not_to have_link(SUBMITTED_TEXT, href: grant_submission_review_path(draft_grant, draft_grant.submissions.first, draft_grant_review))
      end
      ...
      scenario 'it does not include reviews for discarded grants' do
        ...
        expect(page).not_to have_link(ASSIGNED_TEXT, href: grant_submission_review_path(grant1, grant1.submissions.first, grant1_review))
        expect(page).to have_text(grant2.name)
        expect(page).to have_link(SUBMITTED_TEXT, href: grant_submission_review_path(grant2, grant2.submissions.first, grant2_review))
      end
    end
