module ReviewsHelper
  ...
  def display_review_data(review:, user:, grant: review.grant, submission: review.submission)
    ...
    OpenStruct.new(status: review.state,
                   ...
                   due_date: time_tag(review.grant.review_close_date.end_of_day,
                                      date_mmddyyyy(grant.review_close_date)))
  end
  ...
  def review_link_based_on_state(review)
    if review.submitted?
      link_to_if review.grant.published?, display_review_state(review.state),
                 grant_submission_review_path(review.grant, review.submission, review)
    else
      link_to_if review.grant.published?, display_review_state(review.state),
                 edit_grant_submission_review_path(review.grant, review.submission, review)
    end
  end

  def available_to_review_text(available_review_count:, available_submissions_count:)
    text = 'This reviewer can be assigned to review '
    text + if available_submissions_count == 1
             'the following submission.'
           else
             "#{available_review_count} of the following submissions."
           end
  end
end
