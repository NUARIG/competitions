module Grants
  class FormsController < ApplicationController
    ...
    # def index
    #   # authorize :admin, :view_form_builder_forms?
    ...
    def new
      # authorize :admin, :manage_form_builder_forms?
      @form = Submission::Form.new
      authorize @form
    ...
    def edit
      ...
      @form = Submission::Form.includes(:sections=>{:questions=>[:multiple_choice_options]}).find(params[:id])
      authorize @form
    end
    ...
    #   @form = Submission::Form.new(form_params)
    ...
    def update
      ...
      # if @survey.available? && @survey.with_safe_encoding {|o| o.update_attributes_safe_display_order(_params[:form_builder_survey])}
        @form.touch
    ...
    def update_fields
      # authorize :admin, :manage_form_builder_forms?
      @form = Submission::Form.find(params[:form_id])
      authorize @form, :update?
      valid_param = false
      Submission::Form::ALWAYS_EDITABLE_ATTRIBUTES.each do |field|
        if _params[:form_builder_survey][field]
          @form[field] = _params[:form_builder_survey][field]
          valid_param = true
        end
      end
    ...
    # def destroy
    #   # authorize :admin, :manage_form_builder_forms?
    #   @form = Submission::Form.find(params[:id])
    #   authorize @form
    ...
    def import
      raise FormBuilder.new('Survey import method deleted app/controllers/form_builder/survey_controller:107')
      # authorize :admin, :manage_form_builder_forms?
    ...
    def sort_column
      FormBuilder::Survey::SORTABLE_FIELDS.include?(params[:sort_column]) ? params[:sort_column] : 'title'
    end
    ...
    def form_params
      params.require(:submission_form).permit(
          :title,
